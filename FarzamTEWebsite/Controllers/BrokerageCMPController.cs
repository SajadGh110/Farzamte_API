...
namespace FarzamTEWebsite.Controllers
{
    ...
    public class BrokerageCMPController : ControllerBase
    {
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> All_Brokers_On_Date(string Date_Monthly)
        {
            var Transactions = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Select(t => new { t.Brokerage_ID, t.Date_Monthly })
                .Where(t => t.Date_Monthly == Date_Monthly)
                .ToListAsync();

            var Name = await _dbContext.Brokerages
                .AsNoTracking()
                .Where(t => Transactions.Select(tr => tr.Brokerage_ID).Contains(t.Id))
                .ToListAsync();
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> Totals(string Date_Monthly, int id)
        {
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> Chart1(string Date_Monthly, int id)
        {
            ...
            var BOBT_Ranked_Brokerage_Value = BOBT_Brokerage_Value
                ...
                .FirstOrDefault(x => x.Transaction.Brokerage_ID == id);
            // FI_Brokerage_Value
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> Chart2(string Date_Monthly, int id)
        {
            var AllTransactions = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> Chart3(string Date_Monthly, int id)
        {
            var AllTransactions = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> Chart4(string Date_Monthly, int id)
        {
            var AllTransactions = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> Chart5(string Date_Monthly, int id)
        {
            var AllTransactions = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
        ...
        [Authorize]
        [HttpGet]
        public async Task<IActionResult> Top_Brokers(string Date_Monthly)
        {
            var topBrokerAll = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
                .OrderByDescending(t => t.All_Total_Value)
                .Join(_dbContext.Brokerages, t => t.Brokerage_ID, b => b.Id, (t, b) => new { b.Name, t.All_Total_Value })
                .FirstOrDefaultAsync();

            var topBrokerBOBT = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
                .OrderByDescending(t => t.BOBT_Total_Value)
                .Join(_dbContext.Brokerages, t => t.Brokerage_ID, b => b.Id, (t, b) => new { b.Name, t.BOBT_Total_Value })
                .FirstOrDefaultAsync();

            var topBrokerFI = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
                .OrderByDescending(t => t.FI_Total_Value)
                .Join(_dbContext.Brokerages, t => t.Brokerage_ID, b => b.Id, (t, b) => new { b.Name, t.FI_Total_Value })
                .FirstOrDefaultAsync();

            var topBrokerBOBTandFI = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
                .OrderByDescending(t => t.BOBT_AND_FI_Total_Value)
                .Join(_dbContext.Brokerages, t => t.Brokerage_ID, b => b.Id, (t, b) => new { b.Name, t.BOBT_AND_FI_Total_Value })
                .FirstOrDefaultAsync();

            var topBrokerBKI = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
                .OrderByDescending(t => t.BKI_Total_Value)
                .Join(_dbContext.Brokerages, t => t.Brokerage_ID, b => b.Id, (t, b) => new { b.Name, t.BKI_Total_Value })
                .FirstOrDefaultAsync();

            var topBrokerBEI = await _dbContext.Transaction_Statistics_M
                .AsNoTracking()
                .Where(t => t.Date_Monthly == Date_Monthly)
                .OrderByDescending(t => t.BEI_Total_Value)
                .Join(_dbContext.Brokerages, t => t.Brokerage_ID, b => b.Id, (t, b) => new { b.Name, t.BEI_Total_Value })
                .FirstOrDefaultAsync();

            if (topBrokerAll == null && topBrokerBOBT == null && topBrokerFI == null)
            {
            ...
            return Ok(new
            {
                Top_All_Total_Value = new { topBrokerAll?.Name, Total_Value = topBrokerAll?.All_Total_Value },
                Top_BOBT_Total_Value = new { topBrokerBOBT?.Name, Total_Value = topBrokerBOBT?.BOBT_Total_Value },
                Top_FI_Total_Value = new { topBrokerFI?.Name, Total_Value = topBrokerFI?.FI_Total_Value },
                Top_BOBTandFI_Total_Value = new { topBrokerBOBTandFI?.Name, Total_Value = topBrokerBOBTandFI?.BOBT_AND_FI_Total_Value },
                Top_BKI_Total_Value = new { topBrokerBKI?.Name, Total_Value = topBrokerBKI?.BKI_Total_Value },
                Top_BEI_Total_Value = new { topBrokerBEI?.Name, Total_Value = topBrokerBEI?.BEI_Total_Value }
            });
        }
